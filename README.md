# Java Learning Journey

This repository documents my learning process in Java through exercises
and practical projects.

Each project focuses on a specific programming concept and includes its
source code, objectives, and the concepts practiced.

## Projects

| No. | Project | Main topic |
|-----|---------|------------|
|  01 | [Reverse Diagonal Matrix](./01-reverse-diagonal-matrix)|Two-Dimensional Arrays: Concepts, Declaration, Initialization.|
|  02 | [Cinema Seat Manage](./02-cinema-seat-manage)|Two-Dimensional Arrays: Concepts, Declaration, Initialization, Access, Iteration, and Manipulation.|
|  03 | [Console Snake Matrix](./03-console-snake-matrix/)|Two-Dimensional Arrays: Traversal, Manipulation, Movement, Input Validation, Boundary Checking, and Collision Detection.|
|  04 | [Character Battle Simulator](./04-character-battle-simulator/)|Object-Oriented Programming: Classes, Objects, Attributes, Constructors, Methods, Parameters, and Object Interaction.|
|  05 | [Digital Wallet](./05-digital-wallet/)|Object-Oriented Programming: Encapsulation, Static Members, Method Overloading, Validation, and Object Interaction.|
|  06 | [Grade Calculator](./06-grade-calculator/)|Java Swing: GUI Components, Layout Managers, Events, Input Validation, and Interface–Logic Separation.|
|  07 | [Multilingual Study Planner](./07-multilingual-study-planner/)|Java Swing: Selection Components, Event Handling, Validation, Internationalization, Accessibility, and Usability.|
|  08 | 
## Topics

- Variables and data types
- Conditional statements
- Loops
- One-dimensional arrays
- Two-dimensional arrays
- Methods
- Object-oriented programming

## Technologies

- Java
- Apache NetBeans
- Git
- GitHub

## Purpose

The purpose of this repository is to document my progress, strengthen my
programming logic, and build a collection of practical Java projects.
